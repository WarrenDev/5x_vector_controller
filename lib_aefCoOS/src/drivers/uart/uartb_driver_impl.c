...
*
* \brief UARTB driver implementation.
*/
...
#define DRIVER_NAME			(char*)"IUARTB"
#define UART_BASE			UART1
#define UART_INT			UART1_RX_TX_IRQn
#define UART_CLKSRC 		UART1_CLK_SRC
#define DEFAULT_BAUD_RATE	115200
#define DEFAULT_OPEN_PARMS	0
#define BUFFER_SIZE 		256

static device_driver_id_t	driver_id = DRV_IUART_B;
...
void UART1_RX_TX_IRQHandler(void)
{
	...
	if( (status_flags & (kUART_RxDataRegFullFlag | kUART_RxOverrunFlag)) && !buf_isfull(rx_buffer) )
	{
    	 ...
    	 * Handle line mode
    	 */
    	if ( (uart_byte == CHAR_EOL || uart_byte == CHAR_CONT) && (uart_ctx.mode == UARTMODE_LINE) && (uart_ctx.event_handle != NULL ) )
    	{
...
/**
* The UARTB stream interface driver vtable
*/
const stream_driver_vtable_t uartb_vtable =
{
		.getid 		= uart_getid,
