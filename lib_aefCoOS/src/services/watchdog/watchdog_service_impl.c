...
*
* \brief Watchdog system service implementation.
*/
...
#define SERVICE_NAME		(char*)"WATCHDOG"

static service_id_t			service_id = SRV_SYSTEM_WATCHDOG;
...
*/
static
service_id_t system_watchdog_getid (void)
{
	return service_id;
}
...
*/
static
char* system_watchdog_getname (void)
{
	return ctx.name;
}
...
*/
static
service_runlevel_t system_watchdog_runlevel (void)
{
...
*/
static
service_status_t system_watchdog_init (uint32_t init_parameters)
{
	memset (&ctx, 0, sizeof(service_ctx_t));
	ctx.name = SERVICE_NAME;

	return watchdog_core_init (&ctx);
}
...
*/
static
service_status_t system_watchdog_deinit (void)
{
	return watchdog_core_deinit (&ctx);
}
...
static
service_status_t
system_watchdog_iocontrol
(uint32_t code, void* input_buffer, uint32_t input_size, void* output_buffer, uint32_t output_size, uint32_t* bytes_transferred)
{
	return watchdog_core_ioctl (&ctx, code, input_buffer, input_size, output_buffer, output_size, bytes_transferred);
}

/**
* The system watchdog service vtable
*/
const service_vtable_t system_watchdog_srv_vtable =
{
		system_watchdog_getid,
		system_watchdog_getname,
		system_watchdog_runlevel,
		system_watchdog_init,
		system_watchdog_deinit,
		system_watchdog_iocontrol,
};
