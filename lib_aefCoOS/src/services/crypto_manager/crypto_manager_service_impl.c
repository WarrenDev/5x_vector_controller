...
*
* \brief Crypto manager system service implementation.
*/
...
#define SERVICE_NAME		(char*)"CRYPTO"

static service_id_t			service_id = SRV_SYSTEM_CRYPTO;
...
*/
static
service_id_t crypto_manager_getid (void)
{
	return service_id;
}
...
*/
static
char* crypto_manager_getname (void)
{
	return ctx.name;
}
...
*/
static
service_runlevel_t crypto_manager_runlevel (void)
{
...
*/
static
service_status_t crypto_manager_init (uint32_t init_parameters)
{
	memset (&ctx, 0, sizeof(service_ctx_t));
	ctx.name = SERVICE_NAME;

	return crypto_manager_core_init (&ctx);
}
...
*/
static
service_status_t crypto_manager_deinit (void)
{
	return crypto_manager_core_deinit (&ctx);
}
...
static
service_status_t
crypto_manager_iocontrol
(uint32_t code, void* input_buffer, uint32_t input_size, void* output_buffer, uint32_t output_size, uint32_t* bytes_transferred)
{
	return crypto_manager_core_ioctl (&ctx, code, input_buffer, input_size, output_buffer, output_size, bytes_transferred);
}

/**
* The crypto manager service vtable
*/
const service_vtable_t crypto_manager_srv_vtable =
{
		crypto_manager_getid,
		crypto_manager_getname,
		crypto_manager_runlevel,
		crypto_manager_init,
