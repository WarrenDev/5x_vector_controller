...
*
* \brief crypto digital signature ge25519 core implementation.
*
*/
...
static unsigned char 	fe_equal(signed char b, signed char c);
...
fe25519 aa, bbb, cc, dd;
/* See http://www.hyperelliptic.org/EFD/g1p/auto-twisted-extended-1.html#doubling-dbl-2008-hwcd */
void dbl_p1p1(ge25519_p1p1 *r, const ge25519_p2 *p)
{
//	fe25519 a, b, c, d;
	fe25519_square(&aa, &p->x);
	fe25519_square(&bbb, &p->y);
	fe25519_square(&cc, &p->z);
	fe25519_add(&cc, &cc, &cc);
	fe25519_neg(&dd, &aa);

	fe25519_add(&r->x, &p->x, &p->y);
	fe25519_square(&r->x, &r->x);
	fe25519_sub(&r->x, &r->x, &aa);
	fe25519_sub(&r->x, &r->x, &bbb);
	fe25519_add(&r->z, &dd, &bbb);
	fe25519_sub(&r->t, &r->z, &cc);
	fe25519_sub(&r->y, &dd, &bbb);
}
...
unsigned char fe_equal(signed char b, signed char c)
{
	unsigned char ub = b;
	unsigned char uc = c;
...
void choose_t(ge25519_aff *t, unsigned long long pos, signed char b)
{
	...
	*t = ge25519_base_multiples_affine[5 * pos + 0];
	cmov_aff(t, &ge25519_base_multiples_affine[5 * pos + 1], fe_equal(b, 1) | fe_equal(b, -1));
	cmov_aff(t, &ge25519_base_multiples_affine[5 * pos + 2], fe_equal(b, 2) | fe_equal(b, -2));
	cmov_aff(t, &ge25519_base_multiples_affine[5 * pos + 3], fe_equal(b, 3) | fe_equal(b, -3));
	cmov_aff(t, &ge25519_base_multiples_affine[5 * pos + 4], fe_equal(b, -4));
	fe25519_neg(&v, &t->x);
...
static ge25519_p1p1 tp1p1;
static ge25519_p3 pre[16];
static unsigned char bb[127];
