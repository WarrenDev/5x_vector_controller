...
/**
* crypto_message_authentication_512_impl.c
*
* \copyright
...
*
* \brief crypto message authentication 512 implementation.
*/
...
#define	IMPL_MESSAGE_AUTHENTICATION_ID			    MESSAGE_AUTHENTICATION_512
#define IMPL_BLOCK_SIZE							    (SHA512_BLOCK_LENGTH * 8)
...
crypto_status_t message_authentication_init(const uint8_t* key, size_t key_size, message_authentication_ctx_t ctx, size_t* ctx_size)
{
	...
	*/
	int     index;
	uint8_t keyBuffer[SHA512_BLOCK_LENGTH];
	...
	if (key_size > SHA512_BLOCK_LENGTH)
	{
        ...
        sha512_impl->init(&_ctx->sha2_context, &_ctx->sha2_context_size);
        sha512_impl->update(&_ctx->sha2_context, key, key_size);
        sha512_impl->finalize(&_ctx->sha2_context, keyBuffer, &keyBufferSize);
	}
	else
	{
	...
	for (index = 0; index < SHA512_BLOCK_LENGTH; index++)
	{
	...
	*/
    sha512_impl->init(&_ctx->sha2_context, &_ctx->sha2_context_size);
    sha512_impl->update(&_ctx->sha2_context, keyBuffer, keyBufferSize);
	memset(keyBuffer, 0, keyBufferSize);
...
crypto_status_t message_authentication_update(message_authentication_ctx_t ctx, const uint8_t* input, size_t input_size)
{
    ...
    sha512_impl->update(&_ctx->sha2_context, input, input_size);
	return CRYPTO_STATUS_SUCCESS;
}
...
crypto_status_t message_authentication_finalize(message_authentication_ctx_t ctx, const uint8_t* key, size_t key_size, uint8_t* output, size_t* output_size)
{
    ...
    if (*output_size != SHA512_DIGEST_LENGTH)
	{
		*output_size = SHA512_DIGEST_LENGTH;
	...
	*/
	...
	if (key_size > SHA512_BLOCK_LENGTH)
	{
		...
		sha512_impl->init(&local_ctx, &local_ctx_size);
		sha512_impl->update(&local_ctx, key, key_size);
		sha512_impl->finalize(&local_ctx, keyBuffer, &keyBufferSize);
	}
	else
	{
		memcpy(keyBuffer, key, key_size);
	...
	*/
    sha512_impl->finalize(&_ctx->sha2_context, output, output_size);

    sha512_impl->init(&_ctx->sha2_context, &_ctx->sha2_context_size);
    sha512_impl->update(&_ctx->sha2_context, keyBuffer, keyBufferSize);
    sha512_impl->update(&_ctx->sha2_context, output, *output_size);
    sha512_impl->finalize(&_ctx->sha2_context, output, output_size);
	memset(keyBuffer, 0, keyBufferSize);
...
/**
* The message digest vtable for this message authentication digest
*/
const message_authentication_vtable_t message_authentication_512_vtable =
{
