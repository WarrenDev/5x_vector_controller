...
/**
* crypto_message_authentication_256_impl.c
*
* \copyright
...
*
* \brief crypto message authentication 256 implementation.
*/
...
#define	IMPL_MESSAGE_AUTHENTICATION_ID			    MESSAGE_AUTHENTICATION_256
...
crypto_status_t message_authentication_init(const uint8_t* key, size_t key_size, message_authentication_ctx_t ctx, size_t* ctx_size)
{
	...
	*/
	...
	/**
	* Retrieve SHA256 vtable from the crypto registry
	*/
	message_digest_vtable_t* sha256_impl = (message_digest_vtable_t*)crypto_registry_get_vtable(MESSAGE_DIGEST_SHA2_256);
	if (sha256_impl == NULL)
	{
		if ( sha2_256_register() == CRYPTO_STATUS_SUCCESS )
		{ 
			sha256_impl = (message_digest_vtable_t*)crypto_registry_get_vtable(MESSAGE_DIGEST_SHA2_256);
		}
	}
	if (sha256_impl == NULL)
	...
	*/
	...
	if (key_size > SHA256_BLOCK_LENGTH)
	{
        ...
        sha256_impl->init(&_ctx->sha2_context, &_ctx->sha2_context_size);
        sha256_impl->update(&_ctx->sha2_context, key, key_size);
		sha256_impl->finalize(&_ctx->sha2_context, keyBuffer, &keyBufferSize);
	}
	else
	{
	...
	for (index = 0; index < SHA256_BLOCK_LENGTH; index++)
	{
	...
	*/
	sha256_impl->init(&_ctx->sha2_context, &_ctx->sha2_context_size);
    sha256_impl->update(&_ctx->sha2_context, keyBuffer, keyBufferSize);
	memset(keyBuffer, 0, keyBufferSize);
...
crypto_status_t message_authentication_update(message_authentication_ctx_t ctx, const uint8_t* input, size_t input_size)
{
    ...
    sha256_impl->update(&_ctx->sha2_context, input, input_size);
	return CRYPTO_STATUS_SUCCESS;
}
...
crypto_status_t message_authentication_finalize(message_authentication_ctx_t ctx, const uint8_t* key, size_t key_size, uint8_t* output, size_t* output_size)
{
	...
	*/
	...
	if (key_size > SHA256_BLOCK_LENGTH)
	{
		...
		sha256_impl->init(&local_ctx, &local_ctx_size);
		sha256_impl->update(&local_ctx, key, key_size);
		sha256_impl->finalize(&local_ctx, keyBuffer, &keyBufferSize);
	}
	else
	{
		memcpy(keyBuffer, key, key_size);
	...
	*/
	sha256_impl->finalize(&_ctx->sha2_context, output, output_size);

    sha256_impl->init(&_ctx->sha2_context, &_ctx->sha2_context_size);
    sha256_impl->update(&_ctx->sha2_context, keyBuffer, keyBufferSize);
    sha256_impl->update(&_ctx->sha2_context, output, *output_size);
    sha256_impl->finalize(&_ctx->sha2_context, output, output_size);
	memset(keyBuffer, 0, keyBufferSize);
...
*/
const message_authentication_vtable_t message_authentication_256_vtable =
{
