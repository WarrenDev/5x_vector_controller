...
#include <string.h>
#include "aef/cutils/at_parser.h"
...
struct at_parser *at_parser_alloc(const struct at_parser_callbacks *cbs, uint16_t bufsize, void *priv)
{
...
enum at_parser_state at_parser_current_state (struct at_parser *parser)
{
    return parser->state;
}
...
static enum at_response_type generic_line_scanner(const char *line, uint16_t len, struct at_parser *parser)
{
    (void) len;

    if (parser->state == STATE_DATAPROMPT)
        if ( (len == 1) && (memcmp(line, ">", 1) == 0) )
...
static void parser_finalize(struct at_parser *parser)
{
    ...
    if ( (parser->buf_used > 0) && (parser->buf[parser->buf_used] == '\n') )
        parser->buf_used--;
...
static void parser_handle_line(struct at_parser *parser)
{
    ...
    uint16_t len = parser->buf_used - parser->buf_current;
...
//        parser_discard_line(parser); 
    ...
    switch (type & _AT_RESPONSE_TYPE_MASK) {
        ...
        case AT_RESPONSE_FINAL:
        {
            ...
            parser->cbs->handle_response ( line, len, parser->priv );
...
void at_parser_feed(struct at_parser *parser, const void *data, uint16_t len)
{
