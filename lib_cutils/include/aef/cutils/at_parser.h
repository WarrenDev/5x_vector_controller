...
typedef enum at_response_type (*at_line_scanner_t)(const char *line, uint16_t len, void *priv);

/** Response handler. */
typedef void (*at_response_handler_t)(const char *line, uint16_t len, void *priv);
...
typedef struct at_parser_callbacks at_parser_callbacks_t;
